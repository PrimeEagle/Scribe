...
function pp_d_processTables(doc) {
    ...
    for (var i = foundItems.length - 1; i >= 0; i--) {
        ...
        if (paraContents.indexOf("START_TABLE") !== -1 && paraContents.indexOf("END_TABLE") !== -1) {
...
function pp_d_clearParagraphStyleOverrides(doc) {
    for(var s = doc.stories.length - 1; s >= 0; s--) {
        doc.stories[s].clearOverrides(OverrideType.PARAGRAPH_ONLY);
    }
}

function pp_d_clearCharacterStyleOverrides(doc) {
    for(var s = doc.stories.length - 1; s >= 0; s--) {
        doc.stories[s].clearOverrides(OverrideType.CHARACTER_ONLY);
    }
}
...
function pp_pg_moveTables(doc, page) {
